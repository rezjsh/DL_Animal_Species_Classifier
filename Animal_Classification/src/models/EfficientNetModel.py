...
class EfficientNetModel(BaseModel):
    """Concrete strategy for building EfficientNetB0 model."""
    ...
    def build_model(self) -> models.Model:
        logger.info(f"Building EfficientNetB0 model with input shape: {self.input_shape}, "
                    f"include_top={self.include_top}, weights={self.weights}")
        model = EfficientNetB0(input_shape=self.input_shape, include_top=self.include_top, weights=self.weights)
